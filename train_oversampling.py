...
def apply_oversampling(X_train, y_train, target_size=3832, random_state=42):
    ...
    # Calculamos cuántas veces necesitamos repetir los tóxicos
    num_to_add = target_size - len(idx_toxic)
    
    if num_to_add > 0:
        np.random.seed(random_state)
        # Seleccionamos índices aleatorios de los tóxicos existentes para duplicarlos
        idx_oversampled = np.random.choice(idx_toxic, size=num_to_add, replace=True)
        new_idx_toxic = np.concatenate([idx_toxic, idx_oversampled])
    else:
        new_idx_toxic = idx_toxic

    # Combinamos con la clase mayoritaria original
    new_indices = np.concatenate([new_idx_toxic, idx_nontoxic])
    np.random.shuffle(new_indices)
    
    print(f"Oversampling completado: Clase 1 aumentada de {len(idx_toxic)} a {len(new_idx_toxic)}")
